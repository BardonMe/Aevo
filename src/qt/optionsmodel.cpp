...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAevoAmount"))
        settings.setValue("nAnonymizeAevoAmount", 1000);
    nAnonymizeAevoAmount = settings.value("nAnonymizeAevoAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAevoAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAevoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAevoAmount:
            return QVariant(nAnonymizeAevoAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAevoAmount:
            nAnonymizeAevoAmount = value.toInt();
            settings.setValue("nAnonymizeAevoAmount", nAnonymizeAevoAmount);
            emit AnonymizeAevoAmountChanged(nAnonymizeAevoAmount);
            break;
        default:
            break;
        }
