...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAevoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAevoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAevoAmount = strAnonymizeAevoAmount.remove(strAnonymizeAevoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAevoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAevoAmount*COIN) nMaxToAnonymize = nAnonymizeAevoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAevoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAevoAmount));
        strAnonymizeAevoAmount = strAnonymizeAevoAmount.remove(strAnonymizeAevoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAevoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
